...
public class UIOrder : UICanvas
{
    ...
    [SerializeField] private TradeOrder tradeOrderPrefab;
    ...
    public override void Setup()
    {
        ...
        if (isFirstTimeOpen)
        {
            ...
            List<OrderDetail> orderDetails = SaveGameManager.GetOrderDetail();
            foreach (var orderDetail in orderDetails)
            {
                var tradeOrder = Instantiate(tradeOrderPrefab, orderContainer);
                List<DataItem> dataItems = new List<DataItem>();
                List<int> quantities = new List<int>();
                orderDetail.GetItemInOrder(out dataItems, out quantities);
                Debug.Log(dataItems.Count);
                tradeOrder.Setup(dataItems, quantities, orderDetail.price);
            }
        }
    }
